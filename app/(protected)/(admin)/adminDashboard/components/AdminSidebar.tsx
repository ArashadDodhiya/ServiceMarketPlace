// components/AdminSidebar.jsx
"use client";

import Link from "next/link";
import { usePathname } from "next/navigation";
import {
  Users,
  Briefcase,
  ClipboardList,
  DollarSign,
  Settings,
  PieChart,
  LayoutDashboard,
  LogOut,
} from "lucide-react";
import Image from "next/image"; // for profile picture

export default function AdminSidebar() {
  const pathname = usePathname();

  const links = [
    { href: "/adminDashboard", label: "Dashboard Overview", icon: <LayoutDashboard /> },
    { href: "/adminDashboard/users", label: "User Management", icon: <Users /> },
    { href: "/adminDashboard/services", label: "Service Management", icon: <Briefcase /> },
    { href: "/adminDashboard/requests", label: "Request Management", icon: <ClipboardList /> },
    { href: "/adminDashboard/payments", label: "Payment Management", icon: <DollarSign /> },
    { href: "/adminDashboard/analytics", label: "Analytics", icon: <PieChart /> },
    { href: "/adminDashboard/settings", label: "Settings", icon: <Settings /> },
  ];

  return (
    <aside className="w-64 bg-white p-4 shadow-md fixed inset-y-0 md:relative z-10 md:z-auto flex flex-col justify-start h-full">
      {/* Logo Section */}
      <div className="flex items-center justify-center mb-6">
        <Image
          src="/SarvJagat_Logo_Final.png" // Make sure the image path is correct
          alt="Service Marketplace Logo"
          width={40}
          height={40}
          className="rounded"
        />
        <h2 className="text-[20px] font-semibold ml-2">Admin Dashboard</h2>
      </div>
      {/* Navigation Links */}
      <nav className="space-y-2">
        {/* <h2 className="text-xl font-semibold mb-4">AdminDashboard</h2> */}
        {links.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className={`flex items-center p-2 rounded hover:bg-gray-100 transition-colors ${
              pathname === link.href ? "bg-gray-200 font-semibold" : "text-gray-700"
            }`}
          >
            <span className="mr-2">{link.icon}</span>
            {link.label}
          </Link>
        ))}
      </nav>

      {/* Profile Section */}
      <div className="flex items-center p-4 mt-4 bg-gray-100 rounded-lg shadow-inner">
        <Image
          src="data:image/jpeg;base64," // Replace with actual path to profile picture
          alt="Admin Profile Picture"
          width={40}
          height={40}
          className="rounded-full"
        />
        <div className="ml-3">
          <p className="font-semibold text-gray-900">Admin Name</p>
          <button className="flex items-center text-sm text-red-600 hover:underline">
            <LogOut className="mr-1 h-4 w-4" />
            Logout
          </button>
        </div>
      </div>
    </aside>
  );
}
